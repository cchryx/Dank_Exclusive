        ...
        try {
          ...
          if (channel) {
            try {
              ...
              client.users.fetch(giveaway.hostid, false).then((user) => {
                user.send({
              ...
              if (message) {
                  ...
                  .setDescription(
                    `Click ${embedTheme.emoji_join} to enter\n\n${
                      ...
                      giveaway.winnersresults.length > 0
                        ...
                        : `*there was no entries to determine a winner*`
                    }\n${embedTheme.emoji_mainpoint}**Ended:** <t:${Math.floor(
                      giveaway.endsAt / 1000
                    )}:R>\n${
                      embedTheme.emoji_mainpoint
                    }**Lasted For:** \`${humantime(giveaway.duration)}\`\n${
                      embedTheme.emoji_mainpoint
                ...
                const sponsor = await message.guild.members.fetch(
                  giveaway.sponsorid
                );
                ...
                if (giveaway.sponsormessage) {
                  const message_embed = new EmbedBuilder()
                    .setDescription(`**Message:** ${giveaway.sponsormessage}`)
                    .setFooter({
          ...
          if (channel) {
            try {
              ...
              if (message) {
                let display = `${
                  embedTheme.emoji_mainpoint
                }**Ended:** <t:${Math.floor(timer.endsAt / 1000)}:R>\n${
                  embedTheme.emoji_mainpoint
                }**Lasted For:** \`${humantime(timer.duration)}\`\n${
                  embedTheme.emoji_mainpoint
                }**Host:** <@${timer.hostid}>`;
                ...
                for (let i = 0; i < numberofmessages; i++) {
                  ...
                  const mentions_msg = await message.channel.send({
