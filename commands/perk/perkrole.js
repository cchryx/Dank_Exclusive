...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("perkrole")
    .setDescription("Perk command: create/remove/edit your customrole.users")
    .addSubcommand((subcommand) =>
      subcommand
        ...
        .addStringOption((oi) => {
          return oi.setName("color").setDescription("Valid hex color");
        })
    )
    .addSubcommand((subcommand) =>
      subcommand.setName("delete").setDescription("Delete your current role")
    )
    .addSubcommand((subcommand) =>
    ...
    if (interaction.options.getSubcommand() === "create") {
      ...
      if (options.emojiicon) {
        ...
        if (verifyemoji !== false) {
          ...
          if (verifyemoji._emoji.id) {
            ...
            message = "**You emoji was not valid**\n`Couldn't find emoji id`";
            embed.setColor("Red").setDescription(message);
            return verify_msg.edit({ embeds: [embed] });
          }
      ...
      if (slots_used === 0) {
        ...
        slots_display = userData.customrole.users
          .map((user) => {
            const slot_location = userData.customrole.users.indexOf(user) + 1;
      ...
      const roleinfo = {};
      const role = await interaction.guild.roles.fetch(userData.customrole.id);
      ...
      const updaterole = await interaction.guild.roles
        ...
        .catch((error) => {
          console.log(error);
          error_message = `\`${
            error.rawError ? error.rawError.message : "There was an error"
          }\``;
          error_reply(interaction, error_message);
          return false;
        });
      ...
      interaction.guild.roles.delete(userData.customrole.id, "Delete perkrole");
      userData.customrole.id = null;
      userData.customrole.users = [];
      await UserModel.findOneAndUpdate({ userid: userData.userid }, userData);
